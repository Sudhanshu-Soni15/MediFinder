...
if (deleteId != null && !deleteId.trim().isEmpty()) {
  for (int i = 0; i < medicines.size(); i++) {
    Map<String, String> medicine = medicines.get(i);
    if (deleteId.equals(medicine.get("id")) && pharmacyEmail.equalsIgnoreCase(medicine.get("pharmacyEmail"))) {
      medicines.remove(i);
      application.setAttribute("medicines", medicines);
...
if (isPost && request.getParameter("orderId") != null && request.getParameter("orderAction") != null) {
  ...
  for (Map<String, Object> order : orders) {
    if (orderId.equals(String.valueOf(order.get("orderId"))) && pharmacyEmail.equalsIgnoreCase(String.valueOf(order.get("pharmacyEmail")))) {
      String status = String.valueOf(order.get("status"));
      String deliveryType = String.valueOf(order.get("deliveryType"));
      ...
      if ("accept".equals(orderAction) && "PLACED".equals(status)) {
        ...
        order.put("status", "OUT FOR DELIVERY");
...
List<Map<String, String>> pharmacyMedicines = new ArrayList<Map<String, String>>();
List<Map<String, Object>> pharmacyOrders = new ArrayList<Map<String, Object>>();

for (Map<String, String> medicine : medicines) {
  if (pharmacyEmail.equalsIgnoreCase(medicine.get("pharmacyEmail"))) {
    pharmacyMedicines.add(medicine);
  }
}

for (Map<String, Object> order : orders) {
  if (pharmacyEmail.equalsIgnoreCase(String.valueOf(order.get("pharmacyEmail")))) {
