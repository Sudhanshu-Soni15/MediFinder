...
<body class="text-ink" data-search-url="<c:url value='/search.jsp' />">
        ...
        <div class="mx-auto flex max-w-5xl flex-col items-center text-center">
          ...
          <div class="mt-8 w-full max-w-3xl">
            ...
            <form id="searchForm" action="search" method="get" class="glass flex flex-col gap-3 rounded-2xl p-3 shadow-soft sm:flex-row sm:items-center sm:rounded-full">
              ...
              <input type="hidden" id="searchUserLat" name="userLat" value="${sessionScope.userLat}" />
              <input type="hidden" id="searchUserLon" name="userLon" value="${sessionScope.userLon}" />
      ...
      <section class="mx-auto w-full max-w-6xl px-4 pb-6 sm:px-6">
        <div class="rounded-2xl bg-white/80 p-4 shadow-soft sm:p-6">
          <form id="filtersForm" action="search" method="get">
            ...
            <input type="hidden" id="filtersUserLat" name="userLat" value="${sessionScope.userLat}" />
            <input type="hidden" id="filtersUserLon" name="userLon" value="${sessionScope.userLon}" />
          ...
          <div id="resultsGrid" class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            <c:forEach var="med" items="${medicines}">
              <c:url var="detailUrl" value="/MedicineDetailsServlet">
                ...
                <c:param name="userLat" value="${sessionScope.userLat}" />
                <c:param name="userLon" value="${sessionScope.userLon}" />
              </c:url>
              ...
              <article class="result-card flex flex-col gap-4 rounded-xl border border-slate-100 bg-white p-5 shadow-soft transition duration-200 hover:-translate-y-1 hover:shadow-xl">
                ...
                <div class="text-sm text-slate-600">
                  ...
                  <c:choose>
                    ...
                    <c:otherwise><fmt:formatNumber value="${med.distance}" maxFractionDigits="1" /> km</c:otherwise>
                  </c:choose>
                </div>
                <div class="mt-auto">
  ...
  <script>
    (function () {
      ...
      var locationUrl = "<c:url value='/set-location' />";

      function getLocationInputs(name) {
        return document.querySelectorAll("input[name='" + name + "']");
      }

      function hasStoredLocation() {
        var latInput = document.querySelector("#searchForm input[name='userLat']");
        var lonInput = document.querySelector("#searchForm input[name='userLon']");
        return !!(latInput && lonInput && latInput.value && lonInput.value);
      }

      function updateLocationInputs(lat, lon) {
        getLocationInputs("userLat").forEach(function (input) {
          input.value = lat;
        });
        getLocationInputs("userLon").forEach(function (input) {
          input.value = lon;
        });
      }
      ...
      function persistLocation(lat, lon) {
        return fetch(locationUrl, {
          ...
          body: "lat=" + encodeURIComponent(lat) + "&lng=" + encodeURIComponent(lon),
        }).catch(function () {
          return null;
        });
      }
      ...
      function ensureLocation() {
        ...
        locationRequest = new Promise(function (resolve) {
          ...
          navigator.geolocation.getCurrentPosition(function (pos) {
            ...
            persistLocation(lat, lon).finally(function () {
              locationReady = true;
              resolve();
            });
          }, function () {
      ...
      function interceptSubmit(form) {
        ...
        form.addEventListener("submit", function (event) {
          ...
          ensureLocation().finally(function () {
            form.submit();
          });
        });
      }
      ...
      function initLocationForms() {
        locationReady = hasStoredLocation();
        interceptSubmit(document.getElementById("searchForm"));
        interceptSubmit(document.getElementById("filtersForm"));
      }

      if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", initLocationForms);
      } else {
        initLocationForms();
      }
    })();
